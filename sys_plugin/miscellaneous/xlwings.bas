...
Sub ExecuteWindows(IsFrozen As Boolean, Command As String, PYTHON_WIN As String, LOG_FILE As String, SHOW_LOG As Boolean, Optional PYTHONPATH As String)
    ...
    If IsFrozen = False Then
                libdir = "C:\Users\bc0074854\Dropbox\functional\synced_setting_files\Office\Excel\VBA2\src\lib\pypy"
        RunCommand = "python -c ""import sys; sys.path.append(r'" & PYTHONPATH & "'); sys.path.append(r'" & libdir & "'); " & Command & """ "
    ElseIf IsFrozen = True Then
        RunCommand = Command & " "
    End If

        Debug.Print "cmd.exe /C " & DriveCommand & RunCommand & "> " & LOG_FILE
        ' WORKBOOK_FULLNAMEとfrom_xlを削除｡エラーが起こるし､何のために必要か不明だったので｡
    ExitCode = Wsh.run("cmd.exe /C " & DriveCommand & RunCommand & "> " & LOG_FILE, _
